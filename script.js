...
d3.csv("wealth-health-2014.csv", d3.autoType).then(data => {
  ...
  const yScale = d3
    .scaleLinear()
    .domain([
      d3.min(data, d => d.LifeExpectancy),
  ...
  var circles = svg
    ...
    .on("mouseenter", (event, d) => {
      ...
      var tooltip = d3
        ...
        // .style('fill',black)
        .html(
          "<p id='tooltip'>Country: " +
            d.Country +
            "<br> Region: " +
            d.Region +
            "<br> Population: " +
            d3.format(",.2r")(d.Population) +
            "<br> Income: " +
            d3.format(",.2r")(d.Income) +
            "<br> Life Expectancy: " +
            d.LifeExpectancy +
            "</p>"
        );
  ...
  var xAxisDisplay = svg
    .append("g")
    .attr("class", "axis x-axis")
    .call(xAxis)
    .attr("transform", `translate(0, ${height})`);

  var yAxisDisplay = svg
    .append("g")
  ...
  var legend = svg
    ...
    .attr(
      ...
      "translate(" +
        ...
        (margin.top +
          height -
          1.5 * fontSize * colorScale.domain().length -
          margin.bottom * 2) +
        ")"
    );
  // .attr("transform", `translate(0, ${height})`)
